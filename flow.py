        ...
        for new_id, raw_inputs in pending:
            ...
            for inp in resolved:
                if inp.startswith("n:") and inp in self.g.nodes:
                    ...
                    n_edge_added = True
            # Add a structural parent edge whenever no n: dependency edge was
            # created (covers both the empty-inputs fan-out case and the common
            # case where inputs=["USER_QUERY"] — the node still logically
            # descends from src_nid and must appear connected in the DAG).
            if not n_edge_added:
...
def main() -> None:
    ...
    if args and args[0] == "--resume":
        ...
        # Used by the web server to pin the session ID so its SSE stream
        # can find the graph files written by this process.
        new_sid = args[1] if len(args) > 1 else None
        query = " ".join(args[2:]) or "Say hello in one short sentence."
    else:
        query = " ".join(args) or "Say hello in one short sentence."
    asyncio.run(Executor().run(
        query,
        session_id=new_sid or resume_sid,
