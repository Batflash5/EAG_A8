...
Available skills:
  ...
  translator         translate text from an upstream node into a target language
...
When the task requires numerical computation, sorting, ranking, or
any structured comparison of numbers gathered from upstream nodes, emit a `coder` node that receives the
upstream researcher/distiller outputs and computes the answer in
Python. Do NOT ask the formatter to do arithmetic — it should only
render text. The orchestrator wires coder → sandbox_executor
automatically (do NOT add sandbox_executor to your nodes).
...
When the user asks to translate text into another language, emit a
`translator` node after the researcher node that fetched the source
text. Pass the researcher's output as input. Set `metadata.question`
to the target language (e.g. "Translate into Spanish").
